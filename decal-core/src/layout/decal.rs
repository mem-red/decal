...
impl Decal {
    ...
    pub fn to_svg(&self) -> String {
        let mut out = String::with_capacity(self.nodes.len() * 128); // heuristic preallocation
        ...
        self.write_node(NodeId::from(ROOT_ID), &mut out);
        out.push_str("</svg>");
    ...
    fn write_node(&self, node_id: NodeId, out: &mut String) {
        ...
        node.write_svg_start(out);

        for &child_id in &node.children {
            self.write_node(NodeId::from(child_id), out);
        }

        node.write_svg_end(out);
    }
}
